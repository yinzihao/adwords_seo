...
abstract class AdsSoapClient extends SoapClient {
  ...
  private function ProcessResponse($request, $response, $method,
    ...
    $level = isset($e) ? LoggerG::$ERROR : LoggerG::$INFO;
    $this->LogSoapXml($level);
    $this->LogRequestInfo($level);
  }

  /**
   * Logs the SOAP XML to the LoggerG::$SOAP_XML_LOG log after the request has
  ...
  private function LogSoapXml($level) {
    ...
    LoggerG::log(LoggerG::$SOAP_XML_LOG, $message, $level);
  }

  /**
   * Logs the request info to the LoggerG::$REQUEST_INFO_LOG log the request has
  ...
  private function LogRequestInfo($level) {
    ...
    LoggerG::log(LoggerG::$REQUEST_INFO_LOG, $message, $level);
  }
